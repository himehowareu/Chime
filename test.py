...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    if tc is not None:
        com = cmd_run_echoed([sys.executable, "./chime.py",  file_path, *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed([sys.executable, "./chime.py", file_path])
        if com.returncode != 0:
            stats.com_failed += 1
        stats.ignored += 1
...
def run_test_for_folder(folder: str):
    ...
    print()
    print("Compilation failed: %d, Ignored: %d" % ( stats.com_failed, stats.ignored))
    if stats.com_failed != 0:
        exit(1)
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed([sys.executable, "./chime.py", file_path, *tc.argv], input=tc.stdin, capture_output=True)
